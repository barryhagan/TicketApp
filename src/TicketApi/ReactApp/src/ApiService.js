class ApiService {
  ...
  async getSearchResults(searchInput) {
    ...
    const data = await this.getGraphQlData(
      "{ globalSearch(input:" +
        input +
        ") { organizations {id details external_id name tags} tickets {id subject via status external_id tags via priority type organization {name} } users {id external_id name alias email created_at tags} } } "
    );
