...
namespace TicketSearch.LuceneModel
{
    internal static class TicketDoc
    {
        ...
        public static Document GetSearchDoc(Ticket ticket)
        {
            ...
            {
                new StringField(nameof(ticket._id), ticket._id.ToString(), Field.Store.YES),
                new StringField(InMemoryLuceneSearch.DOC_TYPE_FIELD, typeof(Ticket).Name, Field.Store.YES),

                new TextField(nameof(ticket.url), ticket.url ?? InMemoryLuceneSearch.EMPTY_VALUE, Field.Store.NO),
                new TextField(nameof(ticket.external_id), ticket.external_id?.ToString("N") ?? InMemoryLuceneSearch.EMPTY_VALUE, Field.Store.NO),
                ...
                new TextField(nameof(ticket.via), ticket.via ?? InMemoryLuceneSearch.EMPTY_VALUE, Field.Store.NO),
                new TextField(nameof(ticket.has_incidents), ticket.has_incidents.ToString().ToLowerInvariant(), Field.Store.NO),
            ...
            {
                searchDoc.Add(new StringField(nameof(ticket.tags), InMemoryLuceneSearch.EMPTY_VALUE, Field.Store.NO));
            }
